...
} as const satisfies Record<string, readonly PermissionDefinition[]>;

type Catalog = typeof PERMISSION_CATALOG;

/** Permission domains, matching the Prisma `PermissionDomain` enum. */
export type PermissionDomainKey = keyof Catalog;

/** Every valid permission key, as a literal union. */
export type PermissionKey = Catalog[PermissionDomainKey][number]['key'];

/** Keys grantable at platform scope. */
export type PlatformPermissionKey = Catalog['PLATFORM_ADMINISTRATION'][number]['key'];

/** Keys grantable against a single venue. */
export type VenuePermissionKey = Catalog['VENUE_MANAGEMENT'][number]['key'];

/** Keys grantable against a single tournament. */
export type TournamentPermissionKey = Catalog['TOURNAMENT_MANAGEMENT'][number]['key'];
...
export function flattenPermissionCatalog(): Array<
  PermissionDefinition & { domain: PermissionDomainKey }
> {
  return (
    Object.entries(PERMISSION_CATALOG) as Array<
      [PermissionDomainKey, readonly PermissionDefinition[]]
    >
...
export const ALL_PERMISSION_KEYS: readonly PermissionKey[] = flattenPermissionCatalog().map(
  (p) => p.key,
) as PermissionKey[];

/** All keys belonging to a single domain. */
export function permissionKeysForDomain(domain: PermissionDomainKey): PermissionKey[] {
  return PERMISSION_CATALOG[domain].map((p) => p.key) as PermissionKey[];
}
 ...
 */
export const DOMAIN_BY_SCOPE = {
  PLATFORM: 'PLATFORM_ADMINISTRATION',
  VENUE: 'VENUE_MANAGEMENT',
  TOURNAMENT: 'TOURNAMENT_MANAGEMENT',
} as const satisfies Record<string, PermissionDomainKey>;

/** Scope types, matching the Prisma `PermissionScopeType` enum. */
export type PermissionScope = keyof typeof DOMAIN_BY_SCOPE;

/** The domain whose permissions may be granted in `scope`. */
export function domainForScope(scope: PermissionScope): PermissionDomainKey {
  return DOMAIN_BY_SCOPE[scope];
}

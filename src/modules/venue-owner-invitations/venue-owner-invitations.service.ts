...
@Injectable()
export class VenueOwnerInvitationsService {
  private readonly logger = new Logger(VenueOwnerInvitationsService.name);

  constructor(
    private readonly repo: VenueOwnerInvitationsRepository,
  ...
  async listPending(): Promise<VenueOwnerInvitation[]> {
    const rows = await this.repo.listPending();
    return rows.map(mapInvitationToGraphql);
  }

  async invite(input: InviteVenueOwnerInput, actor: AuthUser): Promise<CreateInvitationResult> {
  ...
  async resend(input: ResendVenueOwnerInvitationInput): Promise<CreateInvitationResult> {
  ...
  async revoke(input: RevokeVenueOwnerInvitationInput): Promise<boolean> {
    const row = await this.repo.findById(input.invitationId);
    if (!row) throw new NotFoundException('Invitation not found.');
    if (row.acceptedAt) {
  ...
  async accept(input: AcceptVenueOwnerInvitationInput): Promise<AuthPayload> {
...
— The Arena NP team`;
    ...
    await this.mailer.send({
      to: args.email,
      subject: 'Set up your Arena NP venue-owner account',
      text,
    });
  }
}

...
import { VenueInvitation as PrismaInvitation } from '@prisma/client';

import '../../../common/enums';
import { AdminUser, mapPrismaUserToAdmin } from '../../admin/users/dto/admin-user.model';

@ObjectType({ description: 'Admin-facing view of a pending / accepted venue invitation.' })
...
export class CreateInvitationResult {
  @Field(() => VenueInvitation) invitation!: VenueInvitation;
...
export function mapInvitationToGraphql(row: InvitationWithRelations): VenueInvitation {
  return {
    id: row.id,
    email: row.email,

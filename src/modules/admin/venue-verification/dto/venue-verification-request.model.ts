...
import { CapabilityStatus, VenueVerificationRequest } from '@prisma/client';
 ...
 */
@ObjectType()
export class VenueVerificationRequestModel {
  @Field(() => ID) id!: string;
...
type WithRelations = VenueVerificationRequest & {
  user: Parameters<typeof mapPrismaUserToAdmin>[0];
  reviewer?: Parameters<typeof mapPrismaUserToAdmin>[0] | null;
};

export function mapVenueRequestToGraphql(req: WithRelations): VenueVerificationRequestModel {
  return {
    id: req.id,

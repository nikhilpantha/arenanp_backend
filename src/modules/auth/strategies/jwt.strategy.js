...
var JwtStrategy = function () {
    ...
    var JwtStrategy = _classThis = /** @class */ (function (_super) {
        ...
        function JwtStrategy_1(config, prisma) {
            var _this = this;
            var secret = config.get('app.jwt.accessSecret');
            if (!secret)
                throw new Error('JWT_ACCESS_SECRET is not configured');
            _this = _super.call(this, {
                jwtFromRequest: function (request) {
                    // Try to extract from Authorization header first
                    var token = passport_jwt_1.ExtractJwt.fromAuthHeaderAsBearerToken()(request);
                    if (token)
                        return token;
                    // If not in header, try to extract from accessToken cookie
                    ...
                    switch (_a.label) {
                        ...
                        case 1:
                            ...
                            if (user.tokenVersion !== payload.tokenVersion) {
                                throw new common_1.UnauthorizedException('Session has been invalidated. Please sign in again.');
                            }
                            return [2 /*return*/, {
                                    id: user.id,
                                    phoneNumber: user.phoneNumber,
                                    role: user.role,
                                    capabilities: user.capabilities.map(function (c) { return ({ type: c.type, status: c.status }); }),
